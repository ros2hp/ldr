...
async fn main() -> Result<(), Box<dyn std::error::Error + Sync + Send + 'static>> {
    ...
    let graph_sn = &graph_prefix_wdot[..graph_prefix_wdot.find('.').unwrap()];
    ...
    println!(
        "*** Duration before services shutdown: {:?} secs",
    ...
    println!(
        "*** Duration after services shutdown: {:?} secs",
        Instant::now().duration_since(start).as_secs()
    );
...
struct FlattenRdf<'a> {
    //    pkey: &'a str,                // subject
    ty: &'a types::AttrD, // attribute
    sortk: String,
    value_str: Option<&'a str>,       // scalar value
    value_vstr: Option<Vec<&'a str>>, // used for Dynamo List type and edges (uuids)
}

impl<'a> FlattenRdf<'a> {
    fn new(ty: &'a types::AttrD) -> FlattenRdf<'a> {
...
fn process_node_batch<'a>(
    ...
    // its a service so run forever until channel closed
    loop {

        let chan_batch: Result<_,_>;
        {
            let Ok(mut guard) = alr.lock() else {panic!("Error in acquiring mutex")}; //  wait on channel for some data...
            chan_batch = guard.recv();
        } // guard now out of scope - lock released

        let Ok(batch) = chan_batch else {
                ...
                for attr in node_ty {
                    ...
                    match attr.dt.as_str() {
                        ...
                        "S" => {
                            ...
                            let mut frdf = FlattenRdf::new(attr);
            ...
            bat_w_req = rt_handle.block_on(async {   // no move. bat_w_req moved automatically as it is moved in one of the dynamo API's
                ...
                for (_, v) in &flat_rdf_map {
                    ...
                    for s in (*v).value_vstr.as_ref().unwrap() {
                        ...
                        let mut sk: String = graph_sn.clone(); // TODO: replace with short graph name
                        sk.push('|');
                        sk.push_str(&v.sortk);
                ...
                // m|P
                let mut pfx_node_type = graph_sn.clone();
                pfx_node_type.push('|');
                pfx_node_type.push_str(node_ty.short_nm());
                ...
                for (k,nv) in flat_rdf_map {
		            ...
		            sortk.push_str(graph_sn.as_str());
		            sortk.push('|');
		            ...
		            pfx_attr_name.push_str(graph_sn.as_str());
		            pfx_attr_name.push('|');
                    ...
                    let put =  aws_sdk_dynamodb::types::PutRequest::builder();
                    let putx = put.item("PK", AttributeValue::B(puid_b.clone()))
                        ...
                        "I"|"F" => {
                            ...
                            match nv.value_str {
                                None => {
                                		...
                                		let puty=putx.item("P", AttributeValue::S(pfx_attr_name))
				                        .item("Ty", AttributeValue::S(pfx_node_type.to_string()))
                                        .item("Nul", AttributeValue::Null(true));
				                        match nv.ty.dt.as_str() {
                                            "I" => { puty.item("N", AttributeValue::Null(true)) },
                                            "F" => { puty.item("N", AttributeValue::Null(true)) },
                                            &_ => { todo!() },
                                        }
                                    },
                                Some(v) => {
                                		let puty = putx.item("N", AttributeValue::S(v.to_owned()))
                                		.item("P", AttributeValue::S(pfx_attr_name))
				                        .item("Ty", AttributeValue::S(pfx_node_type.to_string()))
				                        .item("Nul", AttributeValue::Null(false));
				                        match nv.ty.dt.as_str() {
                                            "I" => { puty.item("N", AttributeValue::S(v.to_owned())) },
                                            "F" => { puty.item("N", AttributeValue::S(v.to_owned())) },
                                            &_ => { todo!() },
                                        }
                                    },
                            }
            				        ...
            				        "S" => {
				            let puty = match nv.ty.ix.as_str() {
...
async fn persist_batch(
    ...
    match bat_w_outp {
        Err(err) => {
            panic!("Error in Dynamodb batch write in persist_batch() - {}", err);
        }
        Ok(resp) => {
...
fn gen_sortk(at: &types::AttrD) -> String {
